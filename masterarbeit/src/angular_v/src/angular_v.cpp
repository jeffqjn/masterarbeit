...
int main()
{
    ...
    topics.emplace_back("/camera_right/image_raw");
    ...
    BOOST_FOREACH( rosbag::MessageInstance const  m, view) {
                    ...
                    else if(m.getTopic()=="/camera_right/image_raw")
                    {
                        image.add_raw_image(m.instantiate<sensor_msgs::Image>(),parameters);
                    }
                }
                for (auto n: image.Raw_Image)
                {
                    cv::imshow("a",n.second);
                    cv::waitKey(0);
                }
    //mms->camera
